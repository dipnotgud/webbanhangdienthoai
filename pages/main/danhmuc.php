...
$sql_sanpham = "SELECT * FROM tbl_sanpham WHERE id_danhmuc=$iddanhmuc LIMIT $begin,12";
$query_sanpham = mysqli_query($mysqli, $sql_sanpham);
// Tìm số lượng sản phẩm
$sql_number_product = "SELECT * FROM tbl_sanpham WHERE id_danhmuc=$iddanhmuc";
        ...
        <div class="col l-8">
            ...
            <div class="page_next">
                <button class="page_btn btn_next">
                    <a class="page_link black" href="<?php if($pages>1){ echo 'index.php?quanly=danhmuc&iddanhmuc='.$iddanhmuc.'&pages='.($pages-1);}?>"><i class="fa-solid fa-angles-left"></i></a>
                </button>
                <?php       
                for ($i = 0; $i < $number_pages; $i++) {
                ?>
                    <button class="page_btn btn_current" <?php if($pages==($i+1)){ echo 'style="background: #117369;"';}else{ echo '';} ?>>
                        <a class="page_link" href="index.php?quanly=danhmuc&iddanhmuc=<?php echo $iddanhmuc ?>&pages=<?php echo $i + 1 ?>"><?php echo $i + 1 ?></a>
                    </button>
                <?php
                }
                ?>
                <button class="page_btn btn_next">
                    <a class="page_link black" href="<?php if($pages<$number_pages){ echo 'index.php?quanly=danhmuc&iddanhmuc='.$iddanhmuc.'&pages='.($pages+1);}?>">
